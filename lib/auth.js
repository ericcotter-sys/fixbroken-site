...
function isAdmin(req) {
  return !!(req.session && req.session.user && isAdminEmail(req.session.user.email));
}

// Regenerate the session ID before attaching the user — otherwise a session ID
// planted pre-auth (e.g. via a Domain=.fixbroken.ai cookie from a sibling
// subdomain) stays valid after login (session fixation).
...
function rateLimit({ windowMs = 15 * 60 * 1000, max = 20 } = {}) {
  ...
  return (req, res, next) => {
    ...
    hits.set(key, list);
    if (hits.size > 10000) {
      // Memory cap: drop only expired windows — clearing everything would let
      // a many-IP spray reset its own counters.
...
// Only allow same-site relative redirect targets. Reject "//evil.com" AND
// "/\evil.com" — browsers normalize backslash to slash, turning the latter
...
module.exports = {
  ...
  requireAuth, requireAdmin, requireJson, isAdmin, isAdminEmail, loginSession,
